...
- **排序**〔owner 2026-08-31 在 3001 實測後定的三條：**體文參考母音、上接續與下接續參考子音、`_u` 排在 `u` 之後、`_uu` 排在 `uu` 之後**〕：
  ...
  ⚠️ **記法未定的那一格（M119 的 `暇`）也走同一把碼**〔owner 2026-08-31 指定不要排在最後〕：主鍵借「碼位緊鄰在前、而且有記法的那一個」，於是它落在 `科`(89C8) 之後（`暇` 是 89C9），**排在該段最後而不是整類最後**。
  ⚠️⚠️ **那個位置不是在主張它的音**：`組字` face 是**依載體漢字的日文讀音**排的，治理 §10.9.32 記過「拿相鄰關係去推它的悉曇音是無效的」。格子本身是灰的、點不下去、記法欄顯示 `·`、tooltip 明寫「來源未指明記法」——**視覺上不會被讀成某個子音的成員**。
