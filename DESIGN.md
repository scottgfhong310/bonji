...
- **排序**〔owner 2026-08-31 在 3001 實測後定的三條：**體文參考母音、上接續與下接續參考子音、`_u` 排在 `u` 之後、`_uu` 排在 `uu` 之後**〕：

  ⇒ **只有一份名次表：CBETA 的字母次第**（母音 16 ＋ 子音 35，取自 CBETA 自己的 Big5 碼序），五類全部掛上去，於是「體文第 n 格」與「母音第 n 格」講的是同一個字母。
  · 母音 / 子音 → 直接就是那份名次；· 體文 → 它的母音字母（`;m` → `a;m`、`.h` → `a.h`）；· 上 / 下接續 → 它的子音字母（`k` → `ka`）。
  同一記法多格時以 `rel_element_glyph.fd_sort`（xlsx 那一頁的列號）決勝。

  ⚠️ **`_` 開頭 ＝ 該母音的替代形**，掛在**本體後面**——規則取自 `siddham-converter.js` 的 `EXTRA_VSIGN`（`_u` 的 `base` 就是 `u`），**不是寫死的配對表**；實查 97 個記法裡以 `_` 開頭的恰為 `_u` / `_uu`（`y_u` 的 `_` 在中間，不受影響）。
  ⚠️ **複合接續掛在它第一個子音後面**〔owner 2026-08-31 第二輪〕：取「最長的、本身是子音元件的開頭」——`;nk` → `;n`、`kv`／`ktr` → `k`、`tr`／`t,r` → `t`、`y_u` → `y`。**與 `_` 替代形是同一條原則：衍生形排在本體旁邊，不是丟到最後。** ⚠️ **上接續、下接續、接續擴充三類一起套**——同一條規則只套一半，兩欄並排會出現「一邊在中間、一邊在最後」，比原本更難讀。
  ⚠️ **對不上字母、也拆不出子音開頭的**（`~m`／`:-`）掛在「xlsx `#` 序裡前一個對得上的」後面：`~m` 落在 `au` 之後、`:-` 落在 `.h` 之後。
  ⚠️⚠️ **同一記法多格時以字形自己的碼（`tb_glyph.fd_code`）決勝，不是 xlsx 的列號。** 今天兩者**十組逐項相同**（實測）所以是行為中性的；換掉它是因為**列號不穩定**——owner 2026-08-31 在 Mojikyo 分頁中間插了三列，`#` 就整段往後移了。碼是字型裡的位址，不會因為來源多插幾列而變。
  ⚠️ **記法未定的格子也走同一把碼**〔owner 2026-08-31 指定不要排在最後〕：主鍵借「碼位緊鄰在前、而且有記法的那一個」，**排在該段最後而不是整類最後**。**現況 0 格**（`暇` 已有記法），這條路留著備用。
  ⚠️⚠️ **那個位置不是在主張它的音**：`組字` face 是**依載體漢字的日文讀音**排的，治理 §10.9.32 記過「拿相鄰關係去推它的悉曇音是無效的」。格子本身是灰的、點不下去、記法欄顯示 `·`、tooltip 明寫理由——**視覺上不會被讀成某個子音的成員**。
  ⚠️⚠️ **而 `暇` 這一格正好示範了為什麼那句話要寫下來**：它當初被碼位排在 `科`(89C8) 之後（`暇` 是 89C9），而 owner 2026-08-31 裁定它就是 `jh`——**與 `科` 同音**。**那是巧合，不是那個推論被證實了**：定案的依據是整理者的第一手裁決，庫內至今推導不出來（當組件被用到 0 次）。**一個被否決的論證不會因為結論碰巧對了就變成有效。**
  ⚠️ **不再直接用 `tb_siddham_element.fd_sort` 當主鍵**：該分頁的 `#` 把 `_u` / `_uu` 編在 17 / 18（＝全部之後），而 owner 要的是它們緊接在本體後面。`#` 序退成「對不上字母者」的相對位置依據。
  ⚠️⚠️ **兩群都用 CBETA 那一份名次**——M119 自己的碼序與 CBETA **51 個裡有 44 個名次不同**（它把 `lla;m` 排在子音第一、四個成音節流音排在 `a.h` 之後）。兩群並排時要對得起來，而家族既有的決定就是取 CBETA 次第。
  ⚠️ **上接續 / 下接續在這次改動前就已經零逆序**（實查）——owner 那第 2 條是把規則講明，不是在修一個看得到的錯；真正會動的是體文。
...
- ⚠️⚠️ **本欄的「清除」也一併清空輸入欄**〔owner 2026-09-01〕：兩者是**同一次點擊的兩個產物**，
  清一邊留一邊就對不起來了——而那正是 `clearAll()` 早就兩個一起清的理由，
  **在此之前只有這顆鈕是單邊的**。toast 因此另開一個 key（`toast.compCleared`
  「已清除 Composition（輸入欄也一併清空）」），措辭照既有的 `toast.compUndone`
  ——**共用 `toast.cleared`「已清除」會讓「輸入也沒了」這件事不被講出來**。
  ⚠️ **本欄是空的就什麼都不做**（改跳 `toast.compEmpty`），**不可以順手把輸入清掉**：
  使用者按的是 Composition 的鈕，而那些字與本欄無關（同上一條那句「靜靜退掉一個對不上的
  東西比不退更糟」的鏡像面）。**沒有這道守衛，這顆鈕會變成一個會吃掉手打內容的按鈕。**
  ⚠️⚠️ **實作寫在那顆鈕的處理器、不寫進 `clear()`**：`clear()` 有**兩個呼叫端**，
  而 `bonji.js` 的 `clearAll()` 是在**自己清完輸入之後**才呼叫它（`silent`）
  ——放進 `clear()` 的話那條路會多派發一次 `input`、**在 `clearAll()` 還沒收尾時多跑一次
  `convert()`**。（`undo()` 可以直接碰輸入，是因為它**只有一個呼叫端**；`clear()` 沒有那個條件。）
  實測 `clearAll()` 期間 `#bonji-input` 收到的 `input` 事件數 ＝ **0**。
