...
- Per-field copy buttons (title input + each output); example chips. The input field also has **paste** (replaces the input) and **clear**; if the clipboard cannot be read, the input is selected for you so ⌘V / Ctrl+V replaces it.
...
- **Input assist** (`keyboard` side-tool): a glyph palette **docked on the right** (toggle), **three sets, 624 cells** — `Default set` (278, from `catalog.json`), `Cbeta` (156) and `Mojikyo` (190). The vowels / consonants / vowel-signs / upper- & lower-ligatures of the latter two are exported from `db_siddham` and ordered by the Unicode Siddham alphabet. The two chip rows are **two independent axes** (set × category); only their intersection is shown. Browse by category or search by notation, then click a glyph to insert its ASCII notation at the cursor (re-converts live). Opening it hides the side-tools and pins the palette to the right edge — it overlays the page rather than reflowing it (the input area stays put). A ⋮ button on the panel brings the side-tools back, and × closes it. A footer row inserts space / newline / dash (`-` = word-group separator). For transcribing / collating from source texts: recognize the shape, click, done. The two catalog fonts are read from your local install, so nothing is downloaded.
- **Composition field** (below the input): clicking a `Cbeta` / `Mojikyo` cell in the input assist records that cell's **carrier character** here, while its notation goes into the input above. ⚠️ The font is mapped **per character** (Cbeta → `Siddam`, Mojikyo → `Mojikyo M119`) — both encodings sit on CJK code points, so a single font for the whole field would let one font claim the other's characters and draw a different Siddhaṁ letter that looks perfectly fine. Read-only, with copy / remove-last / clear.
